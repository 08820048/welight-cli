...
import { askConfirm, askPassword, askText, isInteractive } from '../prompt'
...
async function runWizard(): Promise<void> {
  ...
  p.intro(c.bgCyan(c.black(` Welight 配置向导 `)))
  p.log.step(`第一步：配置模型（选提供商 → 选模型 → 填密钥，密钥不回显）`)
  const modelOk = await configureModel()
  if (!modelOk)
    p.log.warn(`模型未配置完整，后面用到模型的功能会不可用。`)

  p.log.step(`其他配置（可全部跳过）`)
  ...
  if (planned.length > 0) {
    p.note(planned.map(([key, value]) => `${key}：${value}`).join(`\n`), `将保存以下配置`)
    const confirmed = await askConfirm(`确认保存？`)
    if (!confirmed) {
      p.log.warn(`已取消，未保存其它配置。`)
    }
    else {
      if (theme)
  ...
  p.outro(`下一步：welight doctor 自检，或 welight ai --chat 通过对话继续配置。`)
}
