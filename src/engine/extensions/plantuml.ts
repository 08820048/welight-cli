 ...
 * 渲染 PlantUML 图表
 */
function renderPlantUMLDiagram(token: any, options: Required<PlantUMLOptions>): string {
  ...
  // 如果启用了内嵌SVG且格式是SVG（异步替换依赖 DOM，仅在主线程执行）
  if (options.inlineSvg && options.format === `svg` && typeof document !== `undefined`) {
...
function createPlantUMLHTML(imageUrl: string, options: Required<PlantUMLOptions>, svgContent?: string): string {
  ...
  // 如果有SVG内容，消毒后嵌入（远程 SVG 不可信，防止 script/事件属性注入）
  if (svgContent) {
    const safeSvgContent = DOMPurify.sanitize(svgContent, {
 ...
 * 生成 PlantUML 扩展
 */
export function markedPlantUML(options: PlantUMLOptions = {}): any {
