 ...
 */
export function markedAlert(options: AlertOptions = {}): any {
  ...
  /**
   * 构建警告块元数据
   * @param variantType 警告类型，如 tip、info 等
   * @param matchedVariant 变体配置，包含图标与默认标题
   * @param fromContainer 是否来自 ::: 容器语法
   */
  ...
  return {
    walkTokens(token: any) {
      ...
      if (matchedVariant) {
        ...
        const firstLine = token.tokens?.[0] as any
        if (firstLine) {
          const rawWithoutType = firstLine.raw?.replace(typeRegexp, ``) ?? ``
          const parts = rawWithoutType.split(`\n`)
          const titlePart = (parts[0] ?? ``).trim()
          if (titlePart) {
            ;(token as any).meta.title = titlePart
            const restRaw = parts.slice(1).join(`\n`).trim()
            if (restRaw) {
              firstLine.raw = restRaw
              firstLine.tokens = marked.Lexer.lexInline(restRaw)
            }
            else {
              token.tokens?.shift()
            }
