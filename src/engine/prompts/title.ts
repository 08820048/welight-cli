 ...
 * Welight AI 快捷语扩展模板。
 *
 * 快捷语在会话消息里保持原文展示，runner 组装模型消息时将其展开为
 * 完整指令模板。匹配按去除首尾空白后的全文精确相等，避免误伤用户的自由输入。
 */

export const TITLE_QUICK_COMMAND = '为这篇文章拟几个吸引人的标题'

export const TITLE_GENERATOR_PROMPT = `# 微信公众号爆款标题生成器
...
2.  **标题评分与排序**：
    *   生成全部候选后，**必须调用 「title_score」 工具**，把候选标题列表传入，由判断层按公众号打开潜力评分（1-5 级）；
    *   按工具返回的得分从高到低排序，在每个标题后附上评分（如 4.2/5）；
    *   为得分最高的 1-3 个标题标记⭐️，表示强烈推荐使用；
    *   仅当工具调用失败时，才退回自行综合评分（满分10分）排序。
...
const QUICK_PROMPT_TEMPLATES: Record<string, string> = {
  [TITLE_QUICK_COMMAND]: TITLE_GENERATOR_PROMPT,
}

export function expandQuickPrompt(prompt: string): string {
  return QUICK_PROMPT_TEMPLATES[prompt] ?? prompt
}
