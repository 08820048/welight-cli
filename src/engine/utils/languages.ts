...
export function hasHighlightLanguage(language: string, hljs: any): boolean {
  ...
  try {
    ...
    console.warn(`[highlight] Failed to read language ${language}; treating it as unavailable:`, error)
    return false
  }
}

export function registerHighlightLanguage(name: string, language: LanguageFn, hljs: any): boolean {
  if (!name || !language || !hljs?.registerLanguage)
    return false
  ...
  try {
    ...
    console.warn(`[highlight] Failed to register language ${name}; skipping it:`, error)
    return false
  }
}
...
export async function loadAndRegisterLanguage(language: string, hljs: any): Promise<void> {
  // 如果已经注册，直接返回
  if (hasHighlightLanguage(language, hljs)) {
    return
  }
  ...
  const loadPromise = (async () => {
    try {
      ...
      if (!registerHighlightLanguage(language, module.default, hljs))
        throw new Error(`Failed to register highlight.js language: ${language}`)
    }
    catch (error) {
...
function safeHighlightValue(text: string, language: string, hljs: any): string {
  try {
    return hljs.highlight(text, { language }).value
  }
  catch (error) {
    console.warn(`[highlight] Failed to highlight ${language}; falling back to escaped plaintext:`, error)
...
export function highlightAndFormatCode(text: string, language: string, hljs: any, showLineNumber: boolean): string {
  ...
  if (showLineNumber) {
    ...
    const highlightedLines = rawLines.map((lineRaw) => {
      const lineHtml = safeHighlightValue(lineRaw, language, hljs)
    ...
    const rawHighlighted = safeHighlightValue(text, language, hljs)
...
export function highlightPendingBlocks(hljs: any, container: Document | Element = document): void {
  ...
  pendingBlocks.forEach((codeBlock) => {
    try {
      ...
      console.warn(`[highlight] Failed to process pending code block; skipping highlight:`, error)
      codeBlock.removeAttribute(`data-language-pending`)
      codeBlock.removeAttribute(`data-raw-code`)
      codeBlock.removeAttribute(`data-show-line-number`)
    }
